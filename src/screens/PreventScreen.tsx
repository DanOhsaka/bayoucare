...
const TIER_CHIP: Record<Tier, string> = {
  ...
  mod: 'bg-warning-bg text-warning-fg',
...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <section className={cn('rounded-lg border border-border bg-card p-6 shadow-[var(--shadow)]', className)}>
      {children}
    </section>
  )
}

function CardHead({ title, chip, chipClass }: { title: string; chip?: string; chipClass?: string }) {
  return (
...
function LungCalculator() {
  ...
  return (
    <Card>
      <CardHead title="🫁 Lung (LDCT)" chip="USPSTF 2021" />
      ...
      <button type="button" onClick={() => setResult(riskLung(v))}
        className="mt-3.5 h-10 w-full rounded-md bg-primary text-sm font-bold text-primary-foreground transition-opacity hover:opacity-90">
        Calculate my risk
      </button>
      ...
      {result && (
        <ResultBlock>
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span className={cn('rounded-full px-2.5 py-1 text-[11px] font-bold', TIER_CHIP[result.tier])}>
...
function BreastCalculator() {
  ...
  return (
    <Card>
      <CardHead title="🎀 Breast" chip="ACS 2023 · family-history based" />
      <div className="mb-2 flex flex-wrap gap-1.5">
      ...
      <button type="button" onClick={() => setResult(riskBreast(v))}
        className="mt-3.5 h-10 w-full rounded-md bg-primary text-sm font-bold text-primary-foreground transition-opacity hover:opacity-90">
        Calculate my risk
      </button>
...
function ColoCalculator() {
  ...
  return (
    <Card>
      <CardHead title="🧫 Colorectal" chip="USPSTF 2021" />
      <div className="mb-2 flex flex-wrap gap-1.5">
      ...
      <button type="button" onClick={() => setResult(riskColo(v))}
        className="mt-3.5 h-10 w-full rounded-md bg-primary text-sm font-bold text-primary-foreground transition-opacity hover:opacity-90">
        Calculate my risk
      </button>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span
              className={cn(
                ...
                result.high ? TIER_CHIP.mod : TIER_CHIP.low,
              )}
            >
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <h3 className="text-base font-semibold text-card-foreground">
            🎯 Risk-stratified screening — your plan, not a generic one
          </h3>
          <span className="rounded-full bg-success-bg px-2.5 py-1 text-xs font-bold text-success-fg">
            USPSTF 2021 · ASCO/ACS-aligned
          </span>
        </div>
        <p className="text-sm text-muted-foreground">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHead title="My screening plan" chip="Up to date" chipClass="bg-success-bg text-success-fg" />
        ...
        <Card>
          <CardHead title="Community screening events" chip="Near Alexandria" chipClass="bg-warning-bg text-warning-fg" />
          <ul className="flex flex-col gap-2">
            {[
        ...
        <Card>
          <CardHead title="Family screening (caregiver view)" />
          <p className="text-sm text-muted-foreground">
        ...
        <Card>
          <CardHead title="Why prevention matters in Louisiana" />
          <p className="text-sm text-muted-foreground">
