...
export interface SelectProps {
  ...
  disabled?: boolean;
  className?: string;
  /** Stable id for the trigger — pairs with a `<label htmlFor>`. */
  id?: string;
  children: ReactNode;
}
...
export function Select({
  ...
  return (
    <SelectContext.Provider value={ctx}>
      {/*
        Elevate the whole field while open so sibling controls (stacked form
        selects) cannot paint over the panel — z-index on the panel alone is
        trapped inside this stacking context.
      */}
      <div
        ref={rootRef}
...
export interface SelectTriggerProps {
  className?: string;
  children: ReactNode;
  /** Accessible name when the visible value alone is not enough. */
  "aria-label"?: string;
}
...
/** Non-selectable section header inside a long list (e.g. month groups). */
export function SelectLabel({
  ...
  return (
    <div
      role="presentation"
      className={cn(
        "sticky top-0 z-[1] bg-background px-2.5 py-1.5 text-[11px] font-semibold tracking-wide text-muted-foreground",
        className,
      )}
    >
      {children}
