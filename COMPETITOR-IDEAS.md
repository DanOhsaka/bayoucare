...
**Status (Sep 11) — finals build:** Ranks 5–8 were already shipped Aug 25 (TrialMatch, vitals bridge, language toggle, mood check) — the Aug 17 status above was stale. Added for the Sept 25 finals:

- **Kreyòl ayisyen + Tiếng Việt are now real locales** (rank 7 was a stub — both options intercepted and snapped back). Four locales at 148-key parity. *Machine-generated translations — get a native speaker to review `checkin.*`, `mood.*` and `vitals.*` before the finals.*
- **New Clinic Ops tab — Ranks 9–13**, carrying brief area 5 (admin burden), the thinnest area before this: prior-auth autopilot, tumor-board prep, Referral Express (mock FHIR R4), PCP decision-support feed, and per-slot no-show prediction with smart rebooking.
- **Parish module extended** with a 14-day abnormal-result follow-up queue and FIT-kit return tracking, scaled off `PARISH_DATA.uns`.
- **Cross-module by design:** Clinic Ops reads Rank 1's `TEAM`/`p._f`/`applied` state and Rank 3's risk engines. Applying "Arrange rides (NEMT)" on the Care Team tab drops Darlene's no-show probability and flips the prior-auth transport criterion. *Probe in patient app* in the CDS feed loads the same inputs into the patient-facing calculator.
- **Rank 3 corrected:** the lung score is now a labelled PLCOm2012-style approximation (not the published Tammemägi coefficients) and the breast card no longer claims Tyrer-Cuzick.
- **Four stage-risk fixes:** EN locale no longer renders Spanish for `vitals.sub`; Alicia is Hodgkin in both `TEAM` and `SURVIVORS`; the 2 am replay resets state so a second click can't duplicate the escalation; `applyLang` re-renders JS-built views via a `RERENDER` registry.

Verified with headless Chrome (42 assertions) plus a stubbed-DOM regression harness (1212 checks, 0 mismatches) and a static integrity pass (89 `getElementById` targets, 0 missing; 4 locales, 0 unbound keys).

**Known issue, needs a decision:** the stylesheet is missing the opening `@media (max-width: …)` for its responsive block, leaving an orphan `}`. Every "mobile" rule therefore applies at all widths — all `.grid2`/`.grid3` collapse to one column, `.ib-row` stacks, `.app-shell` loses its sidebar, and `.badge-days` is hidden. The intended desktop layout is one line away.
